...
#define CREATE_SQL_INDEX_USER_CONFIG    \
    "CREATE UNIQUE INDEX[im_index_key]" \
    "ON[im_user_config]("               \
    "[key]);"
...
void ConfigDao::updateServerIp(const std::string& ip) noexcept { updateKey("serverIp", ip); }

void ConfigDao::updateMsgPort(const uint16_t& msgPort) noexcept { updateKey("gatePort", std::to_string(msgPort)); }

void ConfigDao::updateHttpPort(const uint16_t& httpPort) noexcept { updateKey("httpPort", std::to_string(httpPort)); }
